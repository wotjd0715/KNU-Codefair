...
def dijkstra(start, linked_node_list):  # 모든 노드간 가중치가 1, 이는 추후 바꿀수 있으며 모든 가중치가 1이기에 출발 노드에서 모든 노드까지의 거리 측정가능
    distances = {node: float('inf') for node in range(map.all_node_num)}
    distances[start] = 0
    ...
    while queue:
        ...
        for adjacent in map.linked_node_list[current_node]:
...
def set_weight(node, exit):  # 다익스트라를 통해 탈출구에서 모든 노드까지의 거리를 구한후 비교해서 최솟값 넣기
    for e in exit:
        result = dijkstra(e, map.linked_node_list)
...
def fire_in_node(start_node_num, node, linked_node_list):
    ...
    while queue:
        ...
        for i in map.linked_node_list[prev_node_num]:
            check_node_list.append(i)

        for j in check_node_list:
            ad_node_list = []
            for k in map.linked_node_list[j]:
                if k != prev_node_num:
...
def fire_test(fire_node_num, send_node_num1, send_node_num2, node, linked_node_list):
    ...
    for i in map.linked_node_list[send2]:
    ...
    for i in queue:
        ...
        for j in map.linked_node_list[i]:
            node_list.append(j)
    ...
    for i in queue:
        fire_test(fire_start, send2, i, node, map.linked_node_list)
...
def find_exit(all_node, node, linked_node_list):
    for i in all_node:
        ...
        node[i].set_exit_diret_num(shortest_distance_node_index)
        for k in shortest_distance_node_index:
            shortest_node_num.append(map.linked_node_list[i][k])  # output = shortest node_num
...
if __name__ == "__main__":
    ...
    #length,stairs,width.txt가 이 파일과 같은 경로에 있어야함
    ...
    linked_node_list = []
    for i in range(0, map.all_node_num):
        map.linked_node_list.append([])
        for j in find_linked_node(map.node[i]):
           map.linked_node_list[i].append(j)
    ...
    find_exit(map.all_node, map.node, map.linked_node_list)
    for i in range(0, map.all_node_num):
    ...
    with open('node.txt', 'wb') as f:
        pickle.dump(map.node, f)
    with open('linked_node_list.txt', 'wb') as f:
        pickle.dump(map.linked_node_list, f)
